...
test("keeps production app files free of starter preview wiring", async () => {
  ...
  assert.match(page, /const clientLabelY = chartLabelY\(clientY, volunteerY, height - chart\.bottom\)/);
  assert.match(page, /const volunteerLabelY = chartLabelY\(volunteerY, clientY, height - chart\.bottom\)/);
  assert.match(page, /<stop offset="0%" stopColor="#2563eb" stopOpacity="0\.16" \/>/);
  ...
  assert.match(css, /\.chart-line\.volunteers\s*\{[^}]*stroke:\s*#2563eb;[^}]*stroke-width:\s*1\.75;/s);
  assert.match(css, /\.chart-value-label\.volunteers\s*\{[^}]*font-size:\s*12px;[^}]*font-weight:\s*500;/s);
